...
2. **Malformed chunk recovery** — if a chunked transfer is interrupted or followed by a fresh image command, the partial transfer is aborted and the new command is processed correctly. Previously, all subsequent sequences would be merged into the broken chunk until `m=0`.
