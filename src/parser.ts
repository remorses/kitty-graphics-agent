...
export function extractKittyGraphics(output: string): ParseResult {
  ...
  for (const seq of sequences) {
    ...
    if (chunked) {
      // Malformed chunk recovery: if a continuation chunk contains fresh
      // control keys beyond just m= and q= (the only keys spec allows on
      // continuation chunks), it's a new command — abort the partial transfer.
      // Per spec: "Subsequent chunks must have only the m and optionally q keys."
      const isValidContinuation =
        controlData.format === 32 &&
        controlData.action === '' &&
        controlData.width === 0 &&
        controlData.height === 0 &&
        controlData.medium === 'd' &&
        controlData.imageId === 0
      if (!isValidContinuation) {
        // Abort partial chunked transfer — fall through below to handle
        // this sequence as a fresh single or chunked image
        chunked = undefined
      } else {
        // Valid continuation — accumulate payload
    ...
    // Handle as a new command (first chunk or single image)
    if (controlData.more === 1) {
      chunked = {
        controlData,
