...
Add `kitty-graphics-agent` to the `plugin` array in your `opencode.json`:
...
That's it. OpenCode installs the package automatically at startup via Bun.

The plugin:
1. Sets `AGENT_GRAPHICS=kitty` in the shell environment via the `shell.env` hook
2. Intercepts bash tool output via `tool.execute.after`
3. Strips all Kitty Graphics escape sequences from the text
4. Adds extracted PNG images as file attachments on the tool result
5. The LLM sees the images as media parts in its context
...
```ts
import { extractKittyGraphics } from 'kitty-graphics-agent'
